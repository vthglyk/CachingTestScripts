...
def main():
    ...
    for j in args.input:
        ...
        with open(filename, 'r') as f:
            ...
            if window < step:
                print 'The window (' + str(window) + 's) was smaller than the prometheus query step (' + str(
                    step) + 's)'
                print 'Exiting'
                sys.exit()
            ...
            for i in range(0, len(data)):
                ...
                #     busy_cpu = 0
                #     for j in data[i]['metrics']['cpu0']:
                #         if j['mode'] in ["softirq", "irq", "nice", "system", "user", "guest", "iowait"]:
                #             total_cpu += j['value']
                #             busy_cpu += j['value']
                #         elif j['mode'] in ["steal", "idle"]:
                #             total_cpu += j['value']
                #     cpu_usage_node_exporter.append(busy_cpu / total_cpu * 100)
            ...
            print "Average Cpu Usage Node Exporter = ", str(average_cpu_usage_node_exporter) + '%', str(
                np.average(cpu_squid_in_window_help))
